...
class NotificationService:
    ...
    async def subscribe(self) -> None:
        ...
        await bus.subscribe(UserEvents.DEACTIVATED, self._handle_user_deactivated)
        await bus.subscribe(UserEvents.ROLES_CHANGED, self._handle_user_roles_changed)
        await bus.subscribe(UserEvents.PASSWORD_CHANGED, self._handle_password_changed)
        await bus.subscribe(UserEvents.EMAIL_VERIFIED, self._handle_email_verified)

        # Auth events
        await bus.subscribe(AuthEvents.PASSWORD_RESET_REQUESTED, self._handle_password_reset_requested)
        await bus.subscribe(AuthEvents.PASSWORD_RESET_COMPLETED, self._handle_password_reset_completed)
    ...
    async def unsubscribe(self) -> None:
        ...
        await bus.unsubscribe(UserEvents.DEACTIVATED, self._handle_user_deactivated)
        await bus.unsubscribe(UserEvents.ROLES_CHANGED, self._handle_user_roles_changed)
        await bus.unsubscribe(UserEvents.PASSWORD_CHANGED, self._handle_password_changed)
        await bus.unsubscribe(UserEvents.EMAIL_VERIFIED, self._handle_email_verified)
        await bus.unsubscribe(AuthEvents.PASSWORD_RESET_REQUESTED, self._handle_password_reset_requested)
        await bus.unsubscribe(AuthEvents.PASSWORD_RESET_COMPLETED, self._handle_password_reset_completed)
    ...
    async def _handle_user_updated(self, event: dict[str, Any]) -> None:
        ...
        logger.info("Sending profile updated email for user %s", payload.user_id)
    ...
    async def _handle_password_changed(self, event: dict[str, Any]) -> None:
        ...
        logger.info("Sending password changed email for user %s", payload.user_id)
    ...
    async def _handle_password_reset_requested(self, event: dict[str, Any]) -> None:
        """Handle auth.password_reset_requested - send reset email."""
        payload = PasswordResetRequestedPayload(**event["payload"])
        logger.info("Sending password reset email for user %s", payload.user_id)
        # Reuse existing send_password_reset_email from email_service
        reset_link = f"https://example.com/reset-password?token={payload.reset_token}"
        await send_password_reset_email(payload.email, reset_link)

    async def _handle_password_reset_completed(self, event: dict[str, Any]) -> None:
        """Handle auth.password_reset_completed - send confirmation email."""
        payload = PasswordResetCompletedPayload(**event["payload"])
        logger.info("Sending password reset completed email for user %s", payload.user_id)
    ...
    async def _send_email(self, email) -> None:
        ...
        await _send_via_smtp(email.to_email, email.subject, email.html_body or "")
